...
from decoder import ALL_ADDRESSES, DECODER_VERSION, decode_registers
from modbus_reader import JkBmsModbusReader, ModbusError
from offline_queue import OfflineQueue

LOGGER = logging.getLogger("plts-bms-gateway")
...
def run() -> int:
    ...
    LOGGER.info(
        "BMS gateway mulai | %s %s 8N1 | slave=%s | FC03 %d register individual (0x%04X..0x%04X)",
        config.serial_port,
        config.serial_baud,
        config.slave_id,
        len(ALL_ADDRESSES),
    ...
    try:
        while not stop_event.is_set():
            ...
            try:
                ...
                LOGGER.info(
                    "PACK %.3f V %.3f A %.0f W | SOC %.0f%% | CELLS min=%d max=%d avg=%.0f mV | "
                    "TEMP %.1f/%.1f C | CYCLE %s | QUEUE %s",
                    metrics["pack_voltage_v"],
                    metrics["pack_current_a"],
                    metrics["pack_power_w"],
                    metrics["soc_percent"],
                    min(cells),
                    max(cells),
                    sum(cells) / len(cells),
                    metrics["temperature_1_c"],
                    metrics["temperature_2_c"],
                    metrics["cycle_count"],
                    queue.depth(),
                )
        ...
        LOGGER.info("BMS gateway berhenti | QUEUE %s", queue.depth())
    return 0
