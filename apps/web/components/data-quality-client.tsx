...
export function DataQualityClient({ deviceSlug }: { deviceSlug: string }) {
  ...
  const load = useCallback(() => apiGet<QualityResponse>(`/api/v1/devices/${deviceSlug}/data-quality?date=${date}`).then((value) => { setData(value); setError(null); }).catch((reason) => setError(String(reason))), [date, deviceSlug]);
